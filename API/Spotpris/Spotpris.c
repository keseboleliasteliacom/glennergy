...
int Spotpris_FetchAll(AllaSpotpriser *_AllaSpotpriser)
{
    ...
    char date_str[2][16];
    GetTodayDate(date_str[0], sizeof(date_str[0]));
    GetTomorrowDate(date_str[1], sizeof(date_str[1]));
    ...
    {
        CurlResponse resp;
        json_t *today_array = json_array();
        json_t *tomorrow_array = json_array();
        ...
        for (int k = 0; k < 2; k++)
        {
            ...
            json_array_extend(today_array, root);
            json_decref(root);
        }

        size_t n = json_array_size(today_array);
        ...
        for (size_t j = 0; j < n && j < 128; j++)
        {
            json_t *obj = json_array_get(today_array, j);
